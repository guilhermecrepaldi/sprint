...
import com.sprint.design.SprintTheme
import com.sprint.model.ApiStatus
import com.sprint.model.SessionStatus
import com.sprint.ui.canvas.PlatformMap
import com.sprint.ui.canvas.ZoomableCanvas
import com.sprint.ui.folha.FolhaScreen
import com.sprint.ui.components.SessionSummaryScreen
import com.sprint.ui.tabs.DashboardTab
import com.sprint.ui.tabs.GesturesTab
import com.sprint.ui.tabs.NotebookTab
import com.sprint.ui.tabs.NotesTab
import com.sprint.ui.tabs.PenTab
...
import com.sprint.recognizer.RecognizerChain
import kotlinx.coroutines.launch
import com.sprint.ui.components.LiveLogDialog
...
class MainActivity : ComponentActivity() {
    ...
    override fun dispatchKeyEvent(event: KeyEvent): Boolean {
        if (event.keyCode == KeyEvent.KEYCODE_ENTER && event.action == KeyEvent.ACTION_DOWN) {
            android.util.Log.d("SPRINT_DEBUG", "dispatchKeyEvent ENTER ACTION_DOWN")
            val cb = advanceCallback
            if (cb != null) {
                android.util.Log.d("SPRINT_DEBUG", "advanceCallback found, calling it")
                cb()
                return true
            } else {
                android.util.Log.d("SPRINT_DEBUG", "advanceCallback is null!")
            }
        }
...
fun SprintApp(
    ...
    val handwritingRecognizer = remember { com.sprint.recognizer.MlKitRecognizer(context) }
    ...
    LaunchedEffect(state.status) {
        when (state.status) {
            ...
            SessionStatus.SESSION_SUMMARY -> {
                // Tela de resumo — aguarda ação do usuário
            }
    ...
    LaunchedEffect(selectedTab) {
        if (selectedTab == SprintTab.DASHBOARD) {
            sessionViewModel.fetchHistory()
            sessionViewModel.fetchActivity()
            sessionViewModel.fetchAnalytics()
        }
        if (selectedTab != SprintTab.SPRINT) {
            folhaViewModel.resetForNextFolha()
        }
    }
    val goToSprint: () -> Unit = {
    ...
    SprintTheme(backgroundMode = state.config.backgroundMode) {
        Column(
            modifier = Modifier
                .fillMaxSize()
            ...
            Crossfade(
                ...
                Box(Modifier.fillMaxSize()) {
                    when (tab) {
                        SprintTab.NOTES, SprintTab.SIMULADO -> {
                            Box(Modifier.fillMaxSize()) {
                                when (tab) {
                                    SprintTab.NOTES -> NotesTab(notes = emptyList(), onGoToSprint = goToSprint)
                                    SprintTab.SIMULADO -> com.sprint.ui.tabs.SimuladoTab(
                                        modifier = Modifier.fillMaxSize(),
                                        config = state.config,
                                        sessionViewModel = sessionViewModel,
                                        folhaViewModel = folhaViewModel,
                                        folhaState = folhaState,
                                        onGoToSprint = { selectedTab = SprintTab.SPRINT },
                                    )
                                    else -> {}
                                }
                            }
                    ...
                    SprintTab.DASHBOARD -> DashboardTab(
                        ...
                        fragileSkills = state.fragileSkills,
                        reviewPlan = state.reviewPlan,
                        onGoToSprint = goToSprint,
                        onStartSession = sessionViewModel::startSessionFromDashboard,
                        onRefreshAnalytics = { sessionViewModel.fetchAnalytics() },
                    )

                    SprintTab.MATHTREE -> com.sprint.ui.tabs.TreeTab(
                        modifier = Modifier.fillMaxSize(),
                    ...
                    SprintTab.SPRINT -> {
                        ...
                        if (folha != null) {
                            ...
                            val doAdvance: () -> Unit = {
                                val folhaFields = folha.fields
                                val currentIdx = folhaState.currentExerciseIndex
                                val nextIdx = currentIdx + 1

                                if (nextIdx < folhaFields.size) {
                                    folhaViewModel.activateField(nextIdx)
                                    android.util.Log.d("SPRINT_DEBUG", "doAdvance: avancou para field " + nextIdx + "/" + folhaFields.size)
                                } else {
                                    val latestFolhaState = folhaViewModel.uiState.value
                                    android.util.Log.d("SPRINT_DEBUG", "doAdvance: ultimo field, submettendo folha")
                            ...
                            DisposableEffect(activity) {
                                onDispose { activity?.advanceCallback = null }
                            }
                            ZoomableCanvas(
                                isInSession = true,
                                onAdvance = doAdvance,
                                ...
                                focusContent = {
                                    FolhaScreen(
                                        ...
                                        onConfirmStroke = { fieldIndex, strokes ->
                        ...
                        } else if (state.status == SessionStatus.SESSION_SUMMARY) {
                            val summary = state.sessionSummary ?: return@Box
                            SessionSummaryScreen(
                                ...
                                onReviewNow = { skill ->
                                    folhaViewModel.resetForNextFolha()
                                    sessionViewModel.selectSkill(skill)
                                },
                            )
                        } else if ((state.status == SessionStatus.FINISHED) &&
...
@Composable
fun SprintErrorState(message: String?, onRetry: () -> Unit) {
    val ink = MaterialTheme.colorScheme.onBackground
