...
class FolhaViewModel : ViewModel() {
    ...
    fun activateField(index: Int) {
        _uiState.update { it.copy(activeFieldIndex = index, currentExerciseIndex = index) }
    }
    ...
    fun syncTypedAnswer(folhaId: String, fieldIndex: Int, answer: String) {
        android.util.Log.d("SYNC_DEBUG", "syncTypedAnswer: folhaId=$folhaId fieldIndex=$fieldIndex answer='$answer'")
        _uiState.update { state ->
            val scoped = state.scopedTo(folhaId)
            android.util.Log.d("SYNC_DEBUG", "  before: folhaId=${state.folhaId} typedAnswers=${state.fieldTypedAnswers}")
            val result = scoped.copy(fieldTypedAnswers = scoped.fieldTypedAnswers + (fieldIndex to answer))
            android.util.Log.d("SYNC_DEBUG", "  after: folhaId=${result.folhaId} typedAnswers=${result.fieldTypedAnswers}")
            result
        }
    }
