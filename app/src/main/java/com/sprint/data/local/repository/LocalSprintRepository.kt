...
import com.sprint.data.local.catalog.ExerciseEntity
import com.sprint.data.local.catalog.SprintCatalogDatabase
import com.sprint.data.local.runtime.ExerciseAttemptEntity
import com.sprint.data.local.runtime.PenEventEntity
import com.sprint.data.local.runtime.SessionEntity
import com.sprint.data.local.runtime.SprintRuntimeDatabase
import com.sprint.data.local.runtime.StudentEntity
import com.sprint.data.local.runtime.StudentSkillMemoryEntity
import com.sprint.model.FieldResult
...
import com.sprint.domain.procedural.EloMatchmaker
import com.sprint.domain.procedural.MathBktEngine
import com.sprint.model.HeatmapDay
import com.sprint.model.PenEvent
import com.sprint.model.SessionConfig
import com.sprint.model.SessionStartResponse
import com.sprint.model.SkillProgressItem
import com.sprint.model.SprintHistoryItem
import com.sprint.model.SubmitResult
import com.sprint.model.Thermometer
...
class LocalSprintRepository private constructor(context: Context) {
    ...
    suspend fun createSession(
        sessionId: String,
        studentId: String,
        config: com.sprint.model.SessionConfig,
    ...
    suspend fun submitFolha(
        ...
        // Sprint: max 3 paginas, Simulado: 1 pagina apenas
        val maxPage = if ((config.durationMode == "timed" || config.blindMode)) 0 else 2
        val isFinished = folha.pageIndex >= maxPage
        SubmitResult(
            results = results,
            pageScore = pageScore,
            thermometer = Thermometer(value = pageScore.toDouble(), trend = if (pageScore >= 70) "up" else "down"),
            restartTriggered = false,
            sessionStatus = if (isFinished) "finished" else "active",
            nextFolha = if (isFinished) null else nextFolha(
                studentId = studentId,
                sessionId = sessionId,
                skillTag = skill,
    ...
    private suspend fun nextFolha(
        ...
        val actualSkillTag = if (skillTag == "curriculum_tour") {
            com.sprint.model.MathCurriculum.getFlatNodes().let { nodes -> nodes[pageIndex % nodes.size].id }
        } else {
            skillTag
        }
        ...
        if (!config.simuladoRulesJson.isNullOrEmpty()) {
            try {
                val rules = json.decodeFromString<List<com.sprint.ui.folha.SimuladoSequenceRule>>(config.simuladoRulesJson)
                var globalIndex = 0
                for (rule in rules) {
    ...
    private fun ProceduralExercise.toFolhaField(fieldIndex: Int): FolhaField {
        return FolhaField(
            ...
            answerFormatHint = answerFormatHint,
        )
    }
