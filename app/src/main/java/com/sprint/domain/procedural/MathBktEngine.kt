...
object MathBktEngine {
    ...
    /**
     * Atualiza o foco de erro para uma skill.
     * Se errou com o mesmo tipo do foco atual, incrementa contagem.
     * Se errou com tipo diferente, reinicia o foco.
     * Se acertou, decai a contagem (nunca abaixo de 0).
     */
    fun updateErrorFocus(
        isCorrect: Boolean,
        errorType: String?,
        currentFocus: String?,
        currentCount: Int,
        ...
        if (isCorrect) {
            ...
            return currentFocus to newCount
        }
        if (errorType == currentFocus) {
            return currentFocus to (currentCount + 1)
        }
        return errorType to 1
    }
